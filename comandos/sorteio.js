...
function normalizarCor(cor) {
    ...
    let valor = String(cor).trim().replace("#", "");
...
async function participarSorteio(sorteioId, userId) {
    try {
        const sorteio = await pool.query(
            `
            SELECT *
            FROM sorteios
            WHERE id = $1
        ...
        if (Number(dados.encerra_em) <= Date.now()) {
            return {
                sucesso: false,
